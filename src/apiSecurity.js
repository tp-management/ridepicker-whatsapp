...
export function createApiSecurityMiddleware({
  ...
  return async function apiSecurity(req, res, next) {
    ...
    // Bootstrap authenticates its bearer token inside authRoutes. Keep the
    // exception narrow so a future /api/auth/* route is not accidentally public.
    ...
    // Prototype account discovery/creation was intentionally unauthenticated.
    // Once this middleware is deployed, only verified Supabase Auth bootstrap
    // may bind or create an application account.
